...
test('FormErrors (with register form) renders a snapshot properly', () => {
    const tree = renderer.create(<Router><FormErrors {...registerFormProps}></FormErrors></Router>).toJSON();
...
test('FormErrors (with register form) renders a snapshot properly', () => {
    const tree = renderer.create(<Router> <FormErrors {...loginFormProps} /></Router>).toJSON();
    expect(tree).toMatchSnapshot();
});